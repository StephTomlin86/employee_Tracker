...
viewRoles = () => {
  connection.query(
    "SELECT roles.*, department.name FROM roles LEFT JOIN department ON department.id = roles.department_id",
    (err, res) => {
      if (err) throw err;
...
viewDepartments = () => {
  connection.query("SELECT * FROM department", (err, res) => {
    if (err) throw err;
    console.log(chalk.green("DEPARTMENT"));
    console.table(res);
    start();
  });
};
...
viewEmployees = () => {
  connection.query(
    'SELECT employee.first_name, employee.last_name, roles.title AS "role", managers.first_name AS "manager" FROM employee LEFT JOIN roles ON employee.role_id = roles.id LEFT JOIN employee managers ON employee.manager_id = managers.id GROUP BY employee.id',
    (err, res) => {
      if (err) throw err;
